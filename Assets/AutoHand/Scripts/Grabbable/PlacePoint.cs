    ...
    public class PlacePoint : MonoBehaviour{
        ...
        void CheckPlaceObject(float radius, float scale) {
            if(!disablePlacePointOnPlace &&
                !disableRigidbodyOnPlace &&
                placedObject != null &&
                lastPlacePosition != placedObject.transform.position &&
                !IsStillOverlapping(placedObject, scale)) 
            {
                Remove(placedObject);
            }
        ...
        public virtual void Remove(Grabbable placeObj) {
            ...
            if((!placeObj.parentOnGrab || placedObject.HeldCount() == 0) && parentOnPlace && gameObject.activeInHierarchy) {
        ...
        public virtual void SetStartPlaced() {
            if(startPlaced != null) {
                ...
                //of if it's a prefab that needs to be instantiated
                else {
